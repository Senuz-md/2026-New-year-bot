...
const client = new Client({
    ...
    puppeteer: {
        headless: true,
        // Heroku වල Chrome සොයා ගැනීමට මෙම path එක අනිවාර්ය වේ
...
client.on('ready', () => {
    ...
    // cron: 0 0 0 1 0 * (Seconds, Minutes, Hours, Day, Month[0-11], DayOfWeek)
    const job = schedule.scheduleJob('0 0 0 1 0 *', async function(){ 
        ...
        try {
            ...
            if (fs.existsSync('numbers.txt')) {
                ...
                console.log('numbers.txt ගොනුව සොයාගත නොහැක!');
            }
            
            console.log('සියලුම සුබපැතුම් යවා අවසන්!');

        } catch (error) {
            console.error('වැඩේ කරද්දී අවුලක් වුණා:', error);
        }
    });
});
